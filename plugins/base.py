...
def ACCENT() -> str:
    return _colour("ACCENT", "#00d4aa")


def ACCENT2() -> str:
    return _colour("ACCENT2", "#ff6b35")


def ACCENT3() -> str:
    return _colour("ACCENT3", "#f0c040")


def GREEN() -> str:
    return _colour("GREEN", "#2ed573")


def MUTED() -> str:
    return _colour("MUTED", "#8b949e")
...
class ContestPlugin(ABC):
    ...
    def worked_primary_band_mults(self, qsos: list) -> set:
        ...
        if has_m1:
            return {
                (q["mult1"], q["band"], q["mode"])
                for q in qsos
                if not q["dupe"] and q["is_mult1"] == 1 and q["mult1"]
            }
        return {
            (q["mult1"], q["band"], q["mode"]) for q in qsos if not q["dupe"] and q["mult1"] in ml
        }
    ...
    def worked_secondary_band_mults(self, qsos: list) -> set:
        ...
        return {
            (q["cqz"], q["band"], q["mode"]) for q in qsos if not q["dupe"] and q["cqz"] is not None
        }
