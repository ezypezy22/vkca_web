...
VERSION = "1.88"   # displayed in title bar and header label
...
class App(tk.Toplevel):
    ...
    def _build_ui(self):
        ...
        style.configure("TScrollbar", background=BG3, troughcolor=BG2,
                        bordercolor=BG2, arrowcolor=MUTED)
        ...
        lbl_title = tk.Label(top, text=f"⬡ VK CONTEST ANALYZER  v{VERSION}",
                 font=("Consolas", 15, "bold"), fg=ACCENT, bg=BG)
        lbl_title.pack(side="left")
        self._theme_widgets.append((lbl_title, "label_accent_bg"))
        lbl_by = tk.Label(top, text="by VK2YI",
                 font=("Consolas", 9), fg=MUTED, bg=BG)
        lbl_by.pack(side="left", padx=(6, 0))
        self._theme_widgets.append((lbl_by, "label_muted_bg"))
        self.status_var = tk.StringVar(value="No log loaded  —  click  Load .s3db Log  to begin")
        lbl_status = tk.Label(top, textvariable=self.status_var,
                 font=FONT_S, fg=MUTED, bg=BG)
        lbl_status.pack(side="left", padx=16)
    ...
    def _build_overview_tab(self):
        ...
        # User-adjustable text zoom for the overview panels, on top of the
        # automatic canvas-size scaling in _calc_font_scale(). Persisted in
        # vkcontest_config.json under "overview_zoom" so it survives restarts.
        # Range capped at 1.6 (not higher) since dense panels (_panel_fs)
        # cap their own effective scale at 1.5 to avoid text overlap.
        # Placed first (left-packed) so it can never be pushed off-screen
        # by the panel toggle list, which is the part safe to clip/scroll
        # on narrow windows. Styled boldly/larger than the rest of the
        # dock bar since it's a frequently-used control, not a minor toggle.
        self._ov_zoom = max(0.8, min(1.6, float(self._config.get("overview_zoom", 1.0))))
        zoom_frame = tk.Frame(dock_bar, bg=BG2)
        zoom_frame.pack(side="left", padx=(6, 6), pady=4)
        tk.Label(zoom_frame, text="🔍 TEXT ZOOM", font=("Consolas", 10, "bold"),
                 fg=ACCENT, bg=BG2).pack(side="left", padx=(8, 6))
        ...
        self._ov_zoom_scale = tk.Scale(
            ...
            showvalue=False, length=140, width=14, sliderlength=22,
        ...
        self._ov_zoom_readout.pack(side="left", padx=(0, 8))
        for w in (zoom_frame, self._ov_zoom_scale, self._ov_zoom_readout):
            _Tooltip(w,
                     "Scale text size on the Overview tab independently of "
        ...
        for key in self.PANEL_KEYS:
            ...
            fb.bind("<Enter>",    lambda e, w=fb: w.config(fg=GREEN))
            ...
            cb.bind("<Enter>",    lambda e, w=cb: w.config(fg=ACCENT2))
    ...
    def _on_overview_zoom_change(self, value):
        """Slider callback for the user-adjustable overview text zoom.
    ...
    def _build_missing_tab(self):
        ...
        self.miss_tree.pack(side="left", fill="both", expand=True, padx=(12,0), pady=(0,12))
    ...
    def _build_worked_tab(self):
        ...
        self.work_tree.pack(side="left", fill="both", expand=True, padx=(12,0), pady=(0,12))
        sb.pack(side="right", fill="y", pady=(0,12), padx=(0,12))
    ...
    def _build_bands_tab(self):
        ...
        self.band_tree.pack(side="left", fill="both", expand=True, padx=(12,0), pady=(0,12))
        sb.pack(side="right", fill="y", pady=(0,12), padx=(0,12))
    ...
    def _build_dupes_tab(self):
        ...
        self.dupe_tree.pack(side="left", fill="both", expand=True, padx=(12,0), pady=(0,12))
        sb.pack(side="right", fill="y", pady=(0,12), padx=(0,12))
    ...
    def _build_debug_tab(self):
        ...
        self.debug_tree.pack(side="left", fill="both", expand=True, padx=(12,0), pady=(0,12))
        sb.pack(side="right", fill="y", pady=(0,12), padx=(0,12))
    ...
    def _refresh_overview_chart(self):
        ...
        row_defs = [("gauges",2.0), ("intel",2.4), ("sparks",0.9), ("bars",1.8)]
        vis_rows = []
        ...
        gs = self.fig_overview.add_gridspec(
            ...
            left=0.03, right=0.97, top=0.97, bottom=0.07,
        )
    ...
    def _draw_gauges_on_fig(self, fig, d, total_mults, gs=None, row=0, n_cols=7):
        ...
        def draw_gauge(ax, value, max_val, colour, label, fmt, tooltip=""):
            ...
            frac = min(max(value / max_val, 0), 1) if max_val else 0
            ...
            ax.fill(np.concatenate([ro*np.cos(tt), ri*np.cos(tt[::-1])]),
                    ...
                    color=BG3, zorder=1)
            if frac > 0:
                vt = np.linspace(np.radians(ts - frac*sweep), np.radians(ts), 200)
                ax.fill(np.concatenate([ro*np.cos(vt), ri*np.cos(vt[::-1])]),
                        np.concatenate([ro*np.sin(vt), ri*np.sin(vt[::-1])]),
                        color=colour, zorder=2, alpha=0.92)
                tip_r = (ro + ri) / 2
                ax.plot(tip_r*np.cos(np.radians(ts - frac*sweep)),
                         tip_r*np.sin(np.radians(ts - frac*sweep)),
    ...
    def _draw_block_status_on_ax(self, ax, d):
        ...
        if state == "pre":
            ...
            ax.text(0,  0.5, "CONTEST",  ha="center", va="center", fontsize=11*fs,
                     fontweight="bold", color=MUTED, fontfamily="monospace")
            ax.text(0,  0.0, "OVER",     ha="center", va="center", fontsize=18*fs,
                     fontweight="bold", color=MUTED, fontfamily="monospace")
            ax.text(0, -0.55, f"Ended {end_str}", ha="center", va="center",
                     fontsize=6.5*fs, color=MUTED, fontfamily="monospace")
    ...
    def _draw_band_efficiency_on_ax(self, ax, d):
        ...
        for bar, ns, nq in zip(hbars, n_sh, qsos):
            ax.text(bar.get_width()+0.005, bar.get_y()+bar.get_height()/2,
                     f"{ns}m / {nq}q", va="center", fontsize=6.5*fs,
                     color=MUTED, fontfamily="monospace")
        ax.set_xlim(0, max(effic)*1.5 if effic else 1)
    ...
    def _draw_region_heat_on_ax(self, ax, d):
        ...
        for i, (q, p, w, t) in enumerate(zip(qsos_h, pcts_h, worked_h, total_h)):
            if q > 0:
                ax.text(q + label_gap, i, f"{w}/{t}  ({p:.0f}%)", va="center",
                         fontsize=6.5*fs, color=MUTED, fontfamily="monospace")
        margin = 1.7 + 0.5 * max(0.0, fs - 1.0)
    ...
    def _draw_sparklines_on_gridspec(self, fig, gs, d):
        ...
        for si, (title, data, col) in enumerate(spark_defs):
            ...
            for bh in block_hours:
                # Emphasise day boundaries (every 24h) over plain session
                # boundaries.
                is_day_edge = abs(bh % 24) < 1e-6
                ax_sp.axvline(bh, color=BG3, lw=1.2 if is_day_edge else 0.8,
                               zorder=0)
            ax_sp.fill_between(x, y, alpha=0.20, color=col, zorder=1)
            ax_sp.plot(x, y, color=col, lw=1.6, solid_capstyle="round", zorder=2)
            active = [i for i, v in enumerate(data) if v > 0]
            if active:
                lh = active[-1]
                ax_sp.plot(lh, y[lh], "o", color=col, ms=4, zorder=3)
            ax_sp.text(0.01, 0.97, title, transform=ax_sp.transAxes,
                        fontsize=7*fs, color=MUTED, fontfamily="monospace", va="top")
            ax_sp.text(0.99, 0.97, f"peak {int(peak)}", transform=ax_sp.transAxes,
                        fontsize=7*fs, color=col, fontfamily="monospace", va="top", ha="right")
            ax_sp.set_xlim(0, max(len(data) - 1, 1)); ax_sp.set_ylim(-peak*0.05, peak*1.25)
    ...
    def _draw_region_bars_on_ax(self, ax, d):
        ...
        for xi, (w, col) in enumerate(zip(worked_data, state_bar_cols)):
            b = ax.bar(xi, w, color=col, alpha=0.90, width=0.55)
            bars_w.append(b[0])
        for xi, (w, m, col) in enumerate(zip(worked_data, missing_data, state_bar_cols)):
            ax.bar(xi, m, color=col, alpha=0.15, width=0.55, bottom=w,
        ...
        for bar, w, tot, col in zip(bars_w, worked_data, totals, state_bar_cols):
            if tot > 0 and w > 0:
                if w >= label_h_data:
                    ...
                    ax.text(bar.get_x()+bar.get_width()/2, w + max_total*0.03,
        ...
        for xi, (w, m) in enumerate(zip(worked_data, missing_data)):
            ax.text(xi, w+m + above_gap, f"{w}/{w+m}", ha="center", va="bottom",
                     fontsize=6.5*fs, color=MUTED, fontfamily="monospace")
    ...
    def _refresh_missing(self):
        ...
        for m in p.mult_list():
            ...
            if flt != "ALL" and reg != flt:
                continue
            if m not in worked:
                self.miss_tree.insert("", "end",
                    values=(m, reg, "Target Multiplier is open."))
    ...
    def _refresh_worked(self):
        ...
        for q in reversed(self.log.qso_timeline()):
            block_nr  = "—"
            countdown = "—"
            if cs:
                try:
    ...
    def _refresh_bands(self):
        ...
        for b in sorted(bb.keys()):
            self.band_tree.insert("", "end",
                values=(b, bb[b]["valid"], bb[b]["dupe"], len(bb[b]["mults"])))

    def _refresh_dupes(self):
        for i in self.dupe_tree.get_children():
            self.dupe_tree.delete(i)
        _, bc = self.log.dupe_analysis()
        for call, count in sorted(bc.items(), key=lambda x: x[1], reverse=True)[:25]:
            self.dupe_tree.insert("", "end", values=(call, count))
    ...
    def _build_replay_tab(self):
        ...
        # Speed selector
        tk.Label(ctrl_row, text="  Speed:", font=FONT_S, fg=MUTED, bg=BG2).pack(side="left", padx=(10, 2))
        self._replay_speed_var = tk.StringVar(value="1× (1s)")
        speed_cb = ttk.Combobox(ctrl_row, textvariable=self._replay_speed_var,
                                values=["0.5× (2s)", "1× (1s)", "2× (500ms)", "4× (250ms)"],
                                width=12, state="readonly", font=FONT_B)
        speed_cb.pack(side="left", padx=(0, 12))
    ...
    def _replay_speed_ms(self):
        """Return playback interval in ms from speed combobox."""
        val = self._replay_speed_var.get()
        mapping = {
            "0.5× (2s)":  2000,
            "1× (1s)":    1000,
            "2× (500ms)":  500,
    ...
    def _build_fatigue_tab(self):
        ...
        _style_tree(self._fatigue_roster)
    ...
    def _build_cluster_tab(self):
        ...
        _style_tree(self._cluster_tree)
...
class PluginSplashScreen:
    ...
    def __init__(self, root: tk.Tk, on_done_cb=None):
        self._root    = root
        self._closed  = False
